...
export default {
  ...
  created() {
    this.connectionInfo = this.$route.query
    console.log(this.connectionInfo)
    this.connectionInfo.titleErrorMsg = ''
    this.connectionInfo.connectionUrlErrorMsg = ''
    let that = this
    ipcRenderer.on('update_connection_result', function (event, message) {
      if (typeof message !== 'undefined' && message !== null && message === 1) {
        setItem('updateConnectionInfo', that.connectionInfo)
        that.destroy()
      } else {
        that.$message.error('修改失败，' + message);
      }
    })
  },
  mounted() {
  ...
  methods: {
    ...
    updateConnectionInfo() {
      ...
      if (this.connectionInfo.connectionUrlErrorMsg === '' && this.connectionInfo.titleErrorMsg === '') {
        // setItem('updateConnectionInfo', this.connectionInfo)
        ipcRenderer.send('update_connection_info', this.connectionInfo)
      }
    },
    /**
     * 测试能否访问成功
